...
async function boot() {
  ...
  // ✅ Search button uses unified search flow
  els.searchBtn.onclick = async () => {
    const q = els.barcodeInput.value.trim();
    if (!q) return;
    await searchAndMaybeAdd(q);
  };

  // ✅ Enter key uses unified search flow
...
/* ---------------- Search ---------------- */
async function searchAndMaybeAdd(q) {
  // 1) Try barcode exact match first
  const byBarcode = await window.pos.getProductByBarcode(token, q);
  ...
  // 2) Otherwise search by name/barcode partial list
  await loadProducts(q);

  if ((products || []).length === 0) {
    toast("No product found", "warning");
  }
}
...
async function holdSale() {
  ...
  const r = await window.pos.holdSale(
    token,
...
async function confirmPay() {
  ...
  await tryPrintReceipt({ me, saleResp: r.data, grand, paid });
...
async function tryPrintReceipt({ me, saleResp, grand, paid }) {
  try {
    if (!window.pos || typeof window.pos.printSale !== "function") {
      toast("Printing not available", "danger");
      return;
    }
    ...
    const subtotalNum = cart.reduce(
      (sum, it) => sum + (Number(it.price || 0) * Number(it.qty || 0)),
      0
    );
    const discountNum = Number(discount || 0);
    const finalGrand = Number.isFinite(grand)
      ? Number(grand)
      : Math.max(0, subtotalNum - discountNum);
    ...
    const payload = {
      ...
      total: finalGrand,
      paid: Number(paid || 0),
      change: Math.max(0, Number(paid || 0) - finalGrand),
